...
| 结构化记录 | 共 2,948 条：2,730 条来源对应 + 218 条有法条证据的项目释义；其中 5 条模板/占位来源记录和 2 条存疑译法记录默认不展示，因此默认构建为 2,941 条 |
...
本版构建（`dictionary/build.json`，0.1.0.dev4）包含 **2,941 条默认展示记录、218 条核心学习词条、5,882 条双向学习记录、2,713 项英文候选、简繁各 2,610 行 Rime 词典**。筛选后数量会变化；总结构化记录为 2,948 条，其中 5 条模板/占位和 2 条存疑译法记录默认排除。词条数、译法数、输入法行数和卡片数分别统计，不能互相当作词库规模。
