...
**开发中：0.1.0.dev4。** 当前已有可运行、可验证、可直接下载的数据管线和词库；中国大陆核心学习层已有五批：民商/数据/竞争/知识产权、行政/劳动、税务/贸易、刑事、合同实务词组。**当前最高优先级已经调整为扩充和校对主要中英文法律术语、法律词组与固定搭配，使底层词库本身更准确、更完整。Anki 与 Rime 保留现有导出和测试能力，但暂不作为功能开发重点；私有词库叠加和 Qwen3-ASR/真实音频评测后置。** 完整优先级和续作规则见 [DEVELOPMENT_PLAN.md](DEVELOPMENT_PLAN.md) 与 [PROGRESS.md](PROGRESS.md)，每步记录见 [WORK_LOG.md](WORK_LOG.md)。
...
| 结构化记录 | 共 2,996 条：2,730 条来源对应 + 266 条有法条证据的项目释义；其中 5 条模板/占位来源记录和 2 条存疑译法记录默认不展示，因此默认构建为 2,989 条 |
| 核心学习层 | 266 条中文释义、学习提示、英文项目释译及译法说明；覆盖民法典、公司法、个人信息保护法、反垄断法、著作权法、行政处罚法、行政复议法、劳动合同法、增值税法、关税法、对外贸易法及包含刑法修正案（十二）的现行刑法 |
...
本版构建（`dictionary/build.json`，0.1.0.dev4）包含 **2,989 条默认展示记录、266 条核心学习词条、5,978 条双向学习记录、2,761 项英文候选、简繁各 2,658 行 Rime 词典**。筛选后数量会变化；总结构化记录为 2,996 条，其中 5 条模板/占位和 2 条存疑译法记录默认排除。词条数、译法数、输入法行数和卡片数分别统计，不能互相当作词库规模。
