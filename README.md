...
**开发中：0.1.0.dev3。** 当前已有可运行、可验证、可直接下载的数据管线和词库；中国大陆核心学习层已完成首批民商/数据/竞争/知识产权、第二批行政/劳动以及第三批税务/贸易扩充。刑事核心层、来源参考层逐条编审、私有词库叠加和语音项目适配仍在继续。完成状态和续作入口见 [PROGRESS.md](PROGRESS.md)，每步记录见 [WORK_LOG.md](WORK_LOG.md)。
...
| 结构化记录 | 共 2,913 条：2,730 条来源对应 + 183 条有法条证据的项目释义；其中 5 条模板/占位来源记录默认不展示，因此默认构建为 2,908 条 |
| 核心学习层 | 183 条中文释义、学习提示、英文项目释译及译法说明；覆盖民法典、公司法、个人信息保护法、反垄断法、著作权法、行政处罚法、行政复议法、劳动合同法、增值税法、关税法和对外贸易法 |
...
| 最新远端验证 | GitHub Actions 34667413398：Python 3.10/3.12、单测、构建、真实 Rime、真实 Anki 与下载发布全部成功；自动下载提交 `95e3f0b3be4d89eaff3c3fc973909e0971799d80` 已进入 `main` |
...
第三批税务/贸易层特别区分了容易混淆的概念，例如增值税零税率与免税、税率与征收率、关税纳税人与扣缴义务人、最惠国/协定/特惠/普通税率，以及反倾销、反补贴与保障措施。对《对外贸易法》第八十二条的“单独关税区”只记录法条能够支持的适用范围边界，不自行推定具体地区名单或其他法律后果。
...
核心层来源地区 `INTERNATIONAL` 表示项目编写层，适用法域 `CN` 表示中国大陆法律语境。具体法条版本与出处见 [编审与证据](docs/EDITORIAL.md)；后续官方文本基线见 [中国大陆领域来源基线](docs/CN_DOMAIN_SOURCE_BASELINE_2026-09-12.md)。
...
最新自动发布构建（`dictionary/build.json`，0.1.0.dev3）包含 **2,908 条默认展示记录、183 条核心学习词条、5,816 条双向学习记录、2,682 项英文候选、简繁各 2,583 行 Rime 词典**。筛选后数量会变化；总结构化记录为 2,913 条，其中 5 条模板/占位来源记录默认排除。词条数、译法数、输入法行数和卡片数分别统计，不能互相当作词库规模。
...
当前开发顺序是：**先建立包含刑法修正案（十二）的现行刑法可重复证据批次并编写刑事核心学习层**；随后继续来源参考层逐条编审；再实现私有词库叠加与 Qwen3-ASR/真实音频评测。每个阶段均要求远端 CI、真实 Anki/Rime 验证和下载版发布闭环，详见 [PROGRESS.md](PROGRESS.md) 与 [开发计划](DEVELOPMENT_PLAN.md)。
